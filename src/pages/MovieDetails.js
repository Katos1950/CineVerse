...
export const MovieDetails = () => {
  ...
  useEffect(()=>{
    async function fetchMovie() {
      const respose = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=${process.env.REACT_APP_API_KEY}`);
  ...
  const image = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : imagenotfound;
      ...
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          <p className="my-4">
            ...
            <a className="underline" href={`https://www.imdb.com/title/${movie.imdb_id}`} target="blank" rel="noreferrer">{movie.imdb_id}</a>
          </p>
